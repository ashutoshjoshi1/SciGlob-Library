    ...
    def _build_entry(
        ...
        if "priority" in entry_dict:
            try:
                ...
                raise ScheduleParseError(f"Invalid priority value: {entry_dict['priority']}") from e
    ...
    def _parse_condition(self, condition_str: str) -> tuple[ScheduleCondition, Optional[Any]]:
        ...
        if condition_str.startswith("monthday_"):
            try:
                ...
                raise ScheduleParseError(f"Invalid monthday condition: {condition_str}") from e
